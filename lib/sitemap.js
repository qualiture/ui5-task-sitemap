 ...
 */
module.exports = async function({workspace, log, options, taskUtil}) {
    const {baseUrl, priority = 0.5, changefreq = 'weekly'} = options.configuration;
    
    // Read manifest.json to extract routes
    const manifestResource = await workspace.byGlob('**/manifest.json');
    const manifestContent = await manifestResource[0].getString();
    const manifest = JSON.parse(manifestContent);
    
    // Extract routing configuration
    const routing = manifest['sap.ui5']?.routing;
    if (!routing || !routing.routes) {
        // log.info('No routing configuration found in manifest.json');
        return;
    }
    ...
    const sitemapResource = await taskUtil.resourceFactory.createResource({
        path: '/sitemap.xml',
        string: sitemap
    });

    await workspace.write(sitemapResource);

    
    log.info(`Generated sitemap with ${urls.length} URLs`);
};
